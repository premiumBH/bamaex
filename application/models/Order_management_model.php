...
class Order_management_model extends CI_Model
{
    ...
    public function getPickupStatusOrderSearch($data){
        $this->db->select('order_details.*, 
                            order_payments.*, 
                            order_airway_bill.*,
                            country_table.*,
        ...
        if(isset($data['AWB'])){
            $this->db->like('order_airway_bill.airway_bill', $data['AWB'], 'both');
        }
        if(isset($data['orderDateFrom'])){
            $this->db->or_where('order_details.order_status', $data['orderStatus2']);
        }
        if(isset($data['country'])){
            if(isset($data['AWB'])){
                //$this->db->or_like('country_table.country_name', $data['country'], 'both');
                $this->db->like('country_table.country_name', $data['country'], 'both');
            }else{
                $this->db->like('country_table.country_name', $data['country'], 'both');
            }
        }
