            ...
            <div class="table-container" >
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="filter">
                        ...
                        <td>-----
                            <!--<input type="text" class="form-control form-filter input-sm" name="order_status" id="order_status">--> </td>
                    ...
                    foreach($orders as $order)
                    {
                        ...
                        <tr>
                            ...
                            <?php if(isset($order->CMName) && $orderStatus == 12){?>
                            <td><?php echo $order->CMName;?></td>
                            <?php }?>
                            <?php if(isset($IsShowCourierCol)){?>
                            <td><?php echo ($order->CMID == 0)?'':'Courier Assigned';?></td>
                            <?php }?>
                            <td>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control OrderStatus" id="OrderStatus-<?php echo $order->order_id;?>">
                                        <option value="">Select Status </option>
                                        <?php foreach ($orderStatuses as $orderStatusIn){
                                            if($orderStatusIn->id){?>
                                                <option value="<?php echo $orderStatusIn->id?>"
                                                <?php if($orderStatusIn->id == $order->order_status_id){echo "selected";}?>>
                                                    <?php echo $orderStatusIn->status;?></option>
                                            <?php } }?>
...
<script>
    $(function(){
        ...
        $(".portlet-datatable").on( "change", '.courierMan', function() {
            var orderStatus     = '<?php echo  ($orderStatus == 3)?12:11; ?>';
            var CMID            = $(this).val();
            if(CMID != ''){
                var CMDID            = $(this).attr('id');
                var OrderId         = CMDID.split('CMDID-');
                OrderId             = OrderId[1];
                $('<form action="<?=CTRL?>OrderManagement/assignCourierMan" method="post"><input type="hidden" name="orderId" value="'+OrderId+'"><input type="hidden" name="CMID" value="'+CMID+'"><input type="hidden" name="orderStatus" value="'+orderStatus+'"></form>').appendTo('body').submit();
        ...
        $(".portlet-datatable").on( "change", '.OrderStatus', function() {
            ...
            if(OrderStatus != ''){
                ...
                $('<form action="<?=CTRL?>OrderManagement/changeOrderStatusPickUp" method="post"><input type="hidden" name="orderId" value="'+OrderId+'"><input type="hidden" name="OrderStatusId" value="'+OrderStatus+'"><input type="hidden" name="IsOrderOriginated" value="'+IsOrderOriginated+'"></form>').appendTo('body').submit();
...
<script>
    $(function () {
        $("#regionStatus").change(function () {
            ...
            var IsOrderOriginated    = "<?php echo (isset($IsOrderOriginated))?'yes':'no';?>";
            $.post( "<?=CTRL?>OrderManagement/orderSearch", { orderByRegion: regionStatus, orderStatusId : orderStatusId, orderQuery: orderQuery, IsShowCourierCol:IsShowCourierCol, IsOrderOriginated:IsOrderOriginated }, function( data ) {
    ...
    $(function () {
        $("button.search").click(function () {
            ...
            $.post( "<?=CTRL?>OrderManagement/orderSearch", {
                AWB:AWB,
