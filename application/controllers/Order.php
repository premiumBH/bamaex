...
//            die('HERE');
        ...
        $pdf->SetY(64);
        $pdf->Cell(300, 10, $size." ccm", 0, 0, 'C');

        $pdf->SetY(64);
        $pdf->Cell(350, 10, $waight." KG", 0, 0, 'C');
...
//                die();
            ...
            $address_line = $date['receiver_address_line'];
            $address = $date['receiver_address'];
            $address2 = $date['receiver_state'];
            $city = $date['receiver_city'];
            $mobile = $date['receiver_mobile'];
...
//            if(ctype_alpha(mb_substr($search_input, 0, 5)) and !ctype_alpha(mb_substr($search_input, 5 , strlen($search_input))))
//            {
//               // die('HERE');
//               $search_input = mb_substr($search_input, 5 , strlen($search_input));
//               //die($search_input.'');
        $record = null;
        ...
        //die(print_r($order));
        $order['order'] = $this->removeDuplicate($record);
        $this->load->view('Order/search_result' ,$order);
    ...
    public function view_order()
    {
        ...
        if($sender_details[0]->country_id == $receiver_details[0]->country_id)
        {
            $this->db->select('*');
            $this->db->from('domestic_order_catalog');
            $this->db->join('courier', 'domestic_order_catalog.shipper_id = courier.id');
        ...
        else
        {
            ...
            $this->db->join('courier', 'international_order_catalog.shipper_id = courier.id');
